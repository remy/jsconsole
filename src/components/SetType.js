...
class SetType extends Component {
  ...
  render() {
    ...
    if (shallow && !open) {
      return <div className="type ArrayType closed" onClick={this.toggle}><em>{ displayName }</em><span className="arb-info">({ length })</span></div>;
    }
    ...
    for (let entry of value.entries()) {
      types.push(<Entry key={`setTypeKey-${i+1}`} shallow={true} value={entry} allowOpen={open} />);
      i++;
      if (!open && i === 10) {
        break;
      }
    ...
    if (!open && length > 10) {
      types.push(<span key="setTypeMore-0" className="more arb-info">…</span>);
    }

    if (!open) {
      // intersperce with commas
      types = flatten(zip(types, Array.from({ length: length -1 }, (n, i) => <span key={`sep-${i}`} className="sep">,</span>)));
