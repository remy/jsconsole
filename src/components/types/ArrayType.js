  ...
  render() {
    ...
    if (!open) {
      ...
      for (let i = 0; i < types.length; i++) {
        ...
        if (count !== 0 && !hole) {
          newTypes.push(<span key={`hole-${i}`} className="arb-info">&lt;undefined × { count }&gt;</span>);
          count = 0;
        } else if (hole) {
          count++;
        }
    ...
    if (!open) {
      ...
      types = flatten(zip(types, Array.from({ length: types.length -1 }, (n, i) => <span key={`sep-${i}`} className="sep">,</span>)));

      // do mini output
      return <div className="type ArrayType closed" onClick={this.toggle}><em>Array</em><span className="arb-info">({ length })</span>[ { types } ]</div>;
    }

    // this is the full output view
    return <div className="type ArrayType">
